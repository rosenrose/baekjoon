...
fn main() {
    ...
    let args = [(); 6].map(|_| input.next().unwrap());

    println!("{}", u8::from(big_theta(args)));
}

fn big_theta([a2, a1, a0, c1, c2, n0]: [f64; 6]) -> bool {
    // 0 <= (a2-c1)*n^2 + a1*n + a0 && (a2-c2)*n^2 + a1*n + a0 <= 0
    big_omega([a2, a1, a0, c1, n0]) && big_o([a2, a1, a0, c2, n0])
}
...
fn big_omega([a2, a1, a0, c, n0]: [f64; 5]) -> bool {
    ...
    if is_zero(a_) {
        ...
        if a1.is_sign_negative() {
            return false;
        }

        return n0 >= -a0 / a1;
    }
    if a_.is_sign_negative() {
        return false;
    }
